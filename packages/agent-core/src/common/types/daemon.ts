...
export interface DaemonMethodMap {
  ...
  // Permission
  'permission.respond': { params: PermissionRespondParams; result: undefined };
...
export interface DaemonNotificationMap {
  ...
  // subscribes via `myboteam.onAuthError`. Added alongside the P1
  ...
  // subscribes via `myboteam.onBrowserFrame`. ENG-695 / PR #414 —
  // plan decision #7 explicitly preserves this path.
  'browser.frame': { taskId: string; [key: string]: unknown };

  // MyBoTeam AI credit usage updates (emitted by proxy on each gateway response)
  'myboteam-ai.usage-update': CreditUsage;
