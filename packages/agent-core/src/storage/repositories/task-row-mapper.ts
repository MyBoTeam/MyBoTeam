import type { TaskAttachment, TaskMessage, TaskStatus } from '../../common/types/task.js';
...
export function getMessagesForTask(taskId: string): TaskMessage[] {
  ...
  const messageRows = rowsFromResult<MessageRow>(
    db.exec('SELECT * FROM task_messages WHERE task_id = ? ORDER BY sort_order ASC', [taskId]),
  );
