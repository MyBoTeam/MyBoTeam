/**
 * OpenCodeAdapter — SDK-based runtime bridge between MyBoTeam task lifecycle
...
export class OpenCodeAdapter extends EventEmitter<OpenCodeAdapterEvents> {
  ...
  async startTask(config: TaskConfig): Promise<Task> {
    ...
    //
    // CRITICAL: pass `agent: MYBOTEAM_AGENT_NAME` explicitly.
    ...
    // and silently ignores the entire `myboteam` agent prompt — which
    ...
    // reach the model reliably — not just via `agent.myboteam.prompt`,
   ...
   *
   * The block is deliberately short and forceful — the full MyBoTeam
   * identity + conversational-bypass rules live in `agent.myboteam.prompt`
    ...
    const callbacks: CompletionEnforcerCallbacks = {
      onStartContinuation: async (prompt: string) => {
        if (this.currentSessionId && this.client) {
          // Same `agent: MYBOTEAM_AGENT_NAME` reason as the initial
          // prompt above — continuations are independent SDK calls and
          // must also select the MyBoTeam agent, or the continuation
