...
export interface ResolveTaskConfigOptions {
   ...
   * Forwarded into `buildProviderConfigs` so the MyBoTeam-AI provider can
   * register itself when the runtime is available.
   */
  myboteamRuntime?: MyboteamRuntime;

  /**
   * MyBoTeam AI identity storage deps (injected from the caller's
...
export async function resolveTaskConfig(
  ...
  // 1. Build provider configs. `myboteamRuntime` + `myboteamStorageDeps`
  // forward to the MyBoTeam-AI provider when the optional runtime is loaded
