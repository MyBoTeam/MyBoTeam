...
import type { BundledNodePaths, PlatformConfig } from '../types.js';
...
export function getNodePath(config: PlatformConfig): string {
  ...
  throw new Error(
    `[Bundled Node] Bundled Node.js not found at ${bundled?.nodePath ?? '(unknown path)'}. ` +
      'Run "pnpm -F @myboteam/desktop download:nodejs" and rebuild required artifacts.',
  );
}
  ...
  if (!paths) {
    log.warn(
      '[Bundled Node] Node.js runtime artifacts are missing. ' +
        'Run "pnpm -F @myboteam/desktop download:nodejs" to install them.',
    );
    return;
  }
