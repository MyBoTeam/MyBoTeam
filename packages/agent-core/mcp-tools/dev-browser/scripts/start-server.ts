...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'MyBoTeam', 'dev-browser');
  } else if (process.platform === 'win32') {
    return join(process.env.APPDATA || homeDir, 'MyBoTeam', 'dev-browser');
  } else {
    return join(homeDir, '.myboteam', 'dev-browser');
  }
}
...
const MYBOTEAM_HTTP_PORT = parseInt(process.env.DEV_BROWSER_PORT || '9224', 10);
const MYBOTEAM_CDP_PORT = parseInt(process.env.DEV_BROWSER_CDP_PORT || '9225', 10);

if (!Number.isFinite(MYBOTEAM_HTTP_PORT) || MYBOTEAM_HTTP_PORT < 1 || MYBOTEAM_HTTP_PORT > 65535) {
  throw new Error(
    `Invalid DEV_BROWSER_PORT: ${process.env.DEV_BROWSER_PORT}. Must be a number between 1 and 65535`,
  );
}
if (!Number.isFinite(MYBOTEAM_CDP_PORT) || MYBOTEAM_CDP_PORT < 1 || MYBOTEAM_CDP_PORT > 65535) {
  throw new Error(
...
try {
  const res = await fetch(`http://localhost:${MYBOTEAM_HTTP_PORT}`, {
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          const output = execSync(`netstat -ano | findstr :${MYBOTEAM_HTTP_PORT}`, {
            encoding: 'utf-8',
          });
          ...
          const pid = execSync(`lsof -ti:${MYBOTEAM_HTTP_PORT}`, { encoding: 'utf-8' }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
          }
        }
...
try {
  if (process.platform === 'win32') {
    const output = execSync(`netstat -ano | findstr :${MYBOTEAM_CDP_PORT}`, {
      encoding: 'utf-8',
    });
    const match = output.match(/LISTENING\s+(\d+)/);
    if (match) {
      const pid = match[1];
      console.log(
        `Cleaning up stale Chrome process on CDP port ${MYBOTEAM_CDP_PORT} (PID: ${pid})`,
      );
      execSync(`taskkill /F /PID ${pid}`, { stdio: 'ignore' });
    }
  } else {
    const pid = execSync(`lsof -ti:${MYBOTEAM_CDP_PORT}`, { encoding: 'utf-8' }).trim();
    if (pid) {
      console.log(
        `Cleaning up stale Chrome process on CDP port ${MYBOTEAM_CDP_PORT} (PID: ${pid})`,
      );
...
async function startServer(retry = false): Promise<void> {
  try {
    const server = await serve({
      port: MYBOTEAM_HTTP_PORT,
      cdpPort: MYBOTEAM_CDP_PORT,
      headless,
      profileDir,
