      ...
      switch (name) {
        ...
        case 'browser_snapshot': {
          const {
            ...
            include_history,
            max_tokens,
          } = args as unknown as BrowserSnapshotInput;
          const page = await getPage(page_name);
        ...
        case 'browser_click': {
          const { ref, selector, x, y, position, button, click_count, page_name } =
            args as unknown as BrowserClickInput;
          const page = await getPage(page_name);
        ...
        case 'browser_type': {
          const { ref, selector, text, press_enter, page_name } =
            args as unknown as BrowserTypeInput;
          const page = await getPage(page_name);
          try {
        ...
        case 'browser_scroll': {
          const { direction, amount, ref, selector, position, page_name } =
            args as unknown as BrowserScrollInput;
          const page = await getPage(page_name);
          if (ref) {
            const element = await selectSnapshotRef(page, ref);
            if (!element)
              ...
              if (coords) {
                ...
                return {
                  content: [
                    {
                      type: 'text',
                      text: `Hovered over [ref=${ref}] at (${coords.centerX}, ${coords.centerY}) (coordinate hover: ${hoverCoordApp})`,
                    },
                  ],
                };
              }
        ...
        case 'browser_select': {
          const { ref, selector, value, label, index, page_name } =
            args as unknown as BrowserSelectInput;
          const page = await getPage(page_name);
        ...
        case 'browser_wait': {
          const { condition, selector, script, timeout, page_name } =
            args as unknown as BrowserWaitInput;
          const page = await getPage(page_name);
          ...
          switch (condition) {
            ...
            case 'function': {
              if (!script)
                return {
                  content: [
                    { type: 'text', text: 'Error: "script" is required for function condition' },
                  ],
                  isError: true,
                };
              try {
        ...
        case 'browser_drag': {
          const {
            ...
            target_y,
            page_name,
          } = args as unknown as BrowserDragInput;
          const page = await getPage(page_name);
          const getPos = async (
            ref?: string,
            selector?: string,
          ...
          try {
                ...
                content: [
                  {
                    ...
                    text: el
                      ? `${await el.isVisible()}`
                      : `false (element "${selector}" not found)`,
                  },
                ],
              };
            }
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: toAIFriendlyError(err, ref ? `[ref=${ref}]` : selector || 'element')
                    .message,
                },
              ],
              isError: true,
        ...
        case 'browser_tabs': {
          ...
          } = args as unknown as BrowserTabsInput;
