  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          page_name: { type: 'string', description: 'Optional name for the page (default: "main"). Use different names to manage multiple pages.' },
        },
        required: ['url'],
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          page_name: { type: 'string', description: 'Optional name of the page to snapshot (default: "main")' },
          interactive_only: { type: 'boolean', description: 'If true, only show interactive elements (buttons, links, inputs, etc.). Default: true.' },
          full_snapshot: { type: 'boolean', description: 'Force a complete snapshot instead of a diff. Use after major page changes (modal opened, dynamic content loaded) or when element refs seem incorrect. Default: false.' },
          max_elements: { type: 'number', description: 'Maximum elements to include (1-1000). Default: 300' },
          viewport_only: { type: 'boolean', description: 'Only include elements visible in viewport. Defaults to true for coordinate-click apps (Gmail, Google Drive, etc.), otherwise false.' },
          include_history: { type: 'boolean', description: 'Include navigation history in output. Default: true' },
          max_tokens: { type: 'number', description: 'Maximum estimated tokens (1000-50000). Default: 8000' },
        },
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          position: { type: 'string', enum: ['center', 'center-lower'], description: '"center" clicks viewport center. "center-lower" clicks 2/3 down (preferred for Google Docs).' },
          ...
          button: { type: 'string', enum: ['left', 'right', 'middle'], description: 'Mouse button to click (default: "left"). Use "right" for context menus.' },
          click_count: { type: 'number', description: 'Number of clicks (default: 1). Use 2 for double-click, 3 for triple-click.' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          ref: { type: 'string', description: 'Element ref from browser_snapshot (e.g., "e5"). Preferred over selector.' },
          selector: { type: 'string', description: 'CSS selector to find the input (e.g., "input[name=search]"). Use ref when available.' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          page_name: { type: 'string', description: 'Optional name of the page to screenshot (default: "main")' },
          full_page: { type: 'boolean', description: 'Whether to capture the full scrollable page (default: false, captures viewport only)' },
        },
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          script: { type: 'string', description: 'JavaScript code to execute in the page. Must be plain JS (no TypeScript). Use return to get a value back.' },
          page_name: { type: 'string', description: 'Optional name of the page (default: "main")' },
        },
        required: ['script'],
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['list', 'close'], description: '"list" to get all page names, "close" to close a specific page' },
          page_name: { type: 'string', description: 'Required when action is "close" - the name of the page to close' },
        },
        required: ['action'],
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          text: { type: 'string', description: 'Text to type. Each character is typed with proper key events.' },
          key: { type: 'string', description: 'Special key to press (e.g., "Enter", "Tab", "Escape", "Backspace", "ArrowDown"). Can be combined with modifiers like "Control+a", "Shift+Enter".' },
          typing_delay: { type: 'number', description: 'Delay in ms between keystrokes when typing text (default: 20). Set to 0 for instant typing.' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          actions: { type: 'array', description: 'Array of actions to execute in order', items: { type: 'object', properties: { action: { type: 'string', enum: ['click', 'type', 'snapshot', 'screenshot', 'wait'] }, ref: { type: 'string' }, selector: { type: 'string' }, x: { type: 'number' }, y: { type: 'number' }, text: { type: 'string' }, press_enter: { type: 'boolean' }, full_page: { type: 'boolean' }, timeout: { type: 'number' } }, required: ['action'] } },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['actions'],
      },
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['press', 'type', 'down', 'up'], description: '"press" for key combo (Enter, Meta+v), "type" for raw text character by character, "down"/"up" for hold/release' },
          key: { type: 'string', description: 'Key to press: "Enter", "Tab", "Escape", "Meta+v", "Control+c", "Shift+ArrowDown"' },
          text: { type: 'string', description: 'Text to type character by character (for action="type")' },
          typing_delay: { type: 'number', description: 'Delay in ms between keystrokes when typing text (default: 20). Set to 0 for instant typing.' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['action'],
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          ref: { type: 'string', description: 'Element ref to scroll into view (from browser_snapshot)' },
          selector: { type: 'string', description: 'CSS selector to scroll into view' },
          position: { type: 'string', enum: ['top', 'bottom'], description: 'Scroll to page top or bottom' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
      },
    },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          condition: { type: 'string', enum: ['selector', 'hidden', 'navigation', 'network_idle', 'timeout', 'function'], description: '"selector" waits for element to appear, "hidden" waits for element to disappear, "navigation" waits for page navigation, "network_idle" waits for network to settle, "timeout" waits fixed time, "function" waits for custom JS condition to return true' },
          selector: { type: 'string', description: 'CSS selector (required for "selector" and "hidden" conditions)' },
          script: { type: 'string', description: 'JavaScript expression that returns true when condition is met (required for "function" condition)' },
          timeout: { type: 'number', description: 'Max wait time in ms (default: 30000). For "timeout" condition, this is the wait duration.' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['condition'],
      },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['enter', 'exit'], description: '"enter" to switch into an iframe, "exit" to return to main page' },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          action: { type: 'string', enum: ['list', 'switch', 'close', 'wait_for_new'], description: '"list" shows all tabs, "switch" to tab by index, "close" closes tab by index, "wait_for_new" waits for a popup' },
          index: { type: 'number', description: 'Tab index (0-based) for switch/close actions' },
          timeout: { type: 'number', description: 'Timeout in ms for wait_for_new (default: 5000)' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['action'],
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          position: { type: 'string', enum: ['start', 'current'], description: '"start" jumps to document beginning first (Cmd/Ctrl+Home), "current" types at current cursor position (default: "start")' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['text'],
      },
...
Actions: goto, waitForLoad, waitForSelector, waitForNavigation, findAndFill, findAndClick, fillByRef, clickByRef, snapshot, screenshot, keyboard, evaluate`,
      inputSchema: {
        type: 'object',
        properties: {
          actions: { type: 'array', description: 'Array of actions to execute in order', items: { type: 'object', properties: { action: { type: 'string', enum: ['goto', 'waitForLoad', 'waitForSelector', 'waitForNavigation', 'findAndFill', 'findAndClick', 'fillByRef', 'clickByRef', 'snapshot', 'screenshot', 'keyboard', 'evaluate'] }, url: { type: 'string' }, selector: { type: 'string' }, ref: { type: 'string' }, text: { type: 'string' }, key: { type: 'string' }, pressEnter: { type: 'boolean' }, timeout: { type: 'number' }, fullPage: { type: 'boolean' }, code: { type: 'string' }, skipIfNotFound: { type: 'boolean' } }, required: ['action'] } },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['actions'],
      },
...
Returns JSON only (no snapshots/screenshots) to minimize token usage. Max 20 URLs per call.`,
      inputSchema: {
        type: 'object',
        properties: {
          urls: { type: 'array', items: { type: 'string' }, description: 'Array of URLs to visit and extract data from (1-20 URLs)', maxItems: 20, minItems: 1 },
          extractScript: { type: 'string', description: 'JavaScript code that extracts data from each page. Must return an object.' },
          waitForSelector: { type: 'string', description: 'Optional CSS selector to wait for before running extractScript.' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          enabled: { type: 'boolean', description: 'true to show the highlight glow, false to hide it' },
          page_name: { type: 'string', description: 'Optional page name (default: "main")' },
        },
        required: ['enabled'],
      },
