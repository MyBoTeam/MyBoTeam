...
Core library for the MyBoTeam desktop automation assistant. Provides factory-based APIs for task management, persistent storage, permission handling, logging, and more.

## Usage

This package is internal to the MyBoTeam monorepo and is consumed via workspace dependencies (for example, `"@myboteam/agent-core": "workspace:*"`).

## Quick Start

```typescript
import { createStorage, createTaskManager } from '@myboteam/agent-core';

// Initialize storage (SQLite + encrypted secure storage)
const storage = createStorage({
  databasePath: '/path/to/myboteam.db',
...
```typescript
import { createTaskManager } from '@myboteam/agent-core';
```
...
```typescript
import { createStorage } from '@myboteam/agent-core';
```
...
```typescript
import { createPermissionHandler } from '@myboteam/agent-core';
```
...
```typescript
import { createLogWriter } from '@myboteam/agent-core';
```
...
```typescript
import { createSkillsManager } from '@myboteam/agent-core';
```
...
```typescript
import { createSpeechService } from '@myboteam/agent-core';
```
...
```typescript
import { generateConfig, type BrowserConfig } from '@myboteam/agent-core';
...
- **Node.js >= 20**
- Database: [`sql.js`](https://github.com/sql-js/sql.js) (SQLite compiled to WASM). No native build toolchain needed.
