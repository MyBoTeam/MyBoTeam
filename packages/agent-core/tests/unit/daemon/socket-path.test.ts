...
describe('socket-path', () => {
  ...
  describe('getDaemonDir', () => {
    it('returns ~/.myboteam', () => {
      expect(getDaemonDir()).toBe('/Users/testuser/.myboteam');
    });
  });

  describe('getSocketPath', () => {
    it('falls back to default dir when no dataDir provided', () => {
      expect(getSocketPath()).toBe('/Users/testuser/.myboteam/daemon.sock');
    });
    ...
    it('produces named pipe with hash on Windows', () => {
      ...
      const result = getSocketPath('C:\\Users\\test\\AppData\\MyBoTeam');
      expect(result).toMatch(/^\\\\.\\pipe\\myboteam-daemon-[a-f0-9]{12}$/);
    });
  ...
  describe('getPidFilePath', () => {
    it('falls back to default dir when no dataDir provided', () => {
      expect(getPidFilePath()).toBe('/Users/testuser/.myboteam/daemon.pid');
    });
