...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	const buildMiddlewareStack = client.middlewareStack as {
		add: (
			middleware: BuildMiddleware<object, MetadataBearer>,
			options: { step: "build"; name: string; priority: "low" },
		) => void;
	};
	buildMiddlewareStack.add(middleware, { step: "build", name: "pi-ai-custom-headers", priority: "low" });
}
