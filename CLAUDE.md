...
packages/agent-core/   # Core business logic, types, storage, MCP tools (@myboteam/agent-core, ESM)
```
...
pnpm -F @myboteam/web test:unit            # Web unit tests
pnpm -F @myboteam/web test:integration     # Web integration tests
pnpm -F @myboteam/desktop test:unit        # Desktop main-process unit tests
pnpm -F @myboteam/desktop test:integration # Desktop integration tests
pnpm -F @myboteam/desktop test:e2e:native  # Playwright E2E tests (serial, Electron)
pnpm -F @myboteam/agent-core test       # Agent-core tests
pnpm -F @myboteam/daemon test              # Daemon tests

# Run a single test file
pnpm -F @myboteam/desktop vitest run --config vitest.unit.config.ts path/to/file.unit.test.ts
pnpm -F @myboteam/web vitest run --config vitest.unit.config.ts path/to/file.unit.test.ts
...
# MYBOTEAM_BUNDLED_MCP=1 — bundle MCP tools (used in package/release scripts only)
```
...
3. Typed wrapper in `apps/web/src/client/lib/myboteam.ts`
...
- DB: `myboteam.db` (prod) / `myboteam-dev.db` (dev), in Electron user-data directory
...
- **No root-level test scripts** — always use `-F @myboteam/web`, `-F @myboteam/desktop`, or `-F @myboteam/agent-core`
...
pnpm -F @myboteam/web test:unit          # if apps/web changed
pnpm -F @myboteam/desktop test:unit      # if apps/desktop changed
pnpm -F @myboteam/agent-core test     # if packages/agent-core changed
pnpm -F @myboteam/daemon test            # if apps/daemon changed
```
