...
| Manual/standalone daemon (dev only) | `--data-dir` optional; defaults to `~/.myboteam` for dev       |

**Identity files derived from dataDir:**

| File           | macOS / Linux                   | Windows                                    |
| -------------- | ------------------------------- | ------------------------------------------ |
| Database       | `<dataDir>/myboteam.db`         | `<dataDir>\myboteam.db`                    |
| Socket         | `<dataDir>/daemon.sock`         | `\\.\pipe\myboteam-daemon-<hash(dataDir)>` |
...
sequenceDiagram
    ...
    React->>Preload: myboteam.startTask({ prompt })
...
sequenceDiagram
    ...
    React->>IPC: myboteam.resumeSession(<br/>'sess_abc123', prompt, 'tsk_001')
...
sequenceDiagram
    ...
    alt Hide window (click X)
        ...
        Note over Tray: Tray icon still visible<br/>"MyBoTeam — 1 task running"
    ...
    Note over User: Later... user reopens MyBoTeam
...
sequenceDiagram
    ...
    alt User clicks Cancel
        ...
        alt User clicks Cancel
            ...
            UI->>IPC: myboteam.setCloseBehavior('stop-daemon')
