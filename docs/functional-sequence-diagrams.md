# Functional Sequence Diagrams — MyBoTeam Architecture
...
sequenceDiagram
    ...
    R->>M: window.myboteam.startTask(cfg)<br/>[IPC invoke]
    ...
    GW-->>OC: streamed completion + X-Myboteam-Usage header
...
| Outbound HTTPS            | MyBoTeam LLM Gateway (Free only) and the AI provider                                                       | LLM Gateway proxies, tags per-task, and forwards to the actual model |

> **Note on terminology:** in the PTY era the gate shown inside `opencode serve` was an MCP server MyBoTeam shipped (`file-permission`, `ask-user-question`). In the SDK era it is native OpenCode functionality emitting `permission.asked` / `question.asked` events over the same SSE channel — no MCP server on that hop.
...
sequenceDiagram
    ...
    UI->>Pre: window.myboteam.startTask(cfg)
...
**What this phase does:** everything inside the opencode serve process. MyBoTeam is a passive observer on the SSE side — it never drives the LLM or the tool calls directly, it just reacts to events.
...
The private package `@myboteam/llm-gateway-client` is loaded via dynamic `import()` at daemon startup. In OSS builds the import fails and `noopRuntime` takes over — every call below becomes a no-op except for `isAvailable()` returning `false`. The two diagrams below only make sense in a Free build.
...
sequenceDiagram
    autonumber
    box mistyrose Electron Renderer process
      participant UI as Settings UI<br/>("Use MyBoTeam AI")
    ...
    box honeydew Daemon process
      participant RPC as DaemonRpcServer
      participant RT as MyboteamRuntime<br/>(Free impl, dynamically loaded)
    end
    box lavender Outbound HTTPS
      participant GW as MyBoTeam LLM Gateway
    end

    UI->>Pre: window.myboteam.connectMyboteamAi()
    Pre->>H: ipcRenderer.invoke('myboteam-ai:connect')<br/>[IPC invoke]
    H->>DC: client.call('myboteam-ai.connect')
    DC->>RPC: JSON-RPC 'myboteam-ai.connect'<br/>[JSON-RPC]
    ...
    loop on each gateway response
      GW-->>RT: response header:<br/>X-Myboteam-Usage: { remaining, plan }
      RT->>RPC: rpc.notify('myboteam-ai.usage-update', usage)<br/>[JSON-RPC notify]
...
sequenceDiagram
    ...
    box honeydew Daemon process
      participant TS as TaskService
      participant A as OpenCodeAdapter
      participant RT as MyboteamRuntime
    end
    box oldlace opencode serve subprocess
      participant OC as opencode serve
    end
    ...
    GW-->>OC: streamed completion +<br/>X-Myboteam-Usage header
...
- **OpenCode doesn't know about the gateway.** From `opencode serve`'s point of view it is calling a normal provider endpoint — the swap happens inside the provider config that `buildMyboteamAiConfig` emits. That's why the integration survives OpenCode SDK upgrades without changes.
...
- **OSS parity.** In the OSS build `setProxyTaskId` is `undefined` (optional-chain short-circuits), `buildMyboteamAiConfig` returns empty, and `myboteam-ai.*` RPCs throw `myboteam_runtime_unavailable`. None of these diagrams' Free-specific arrows fire.
...
### 4.1 Local ports MyBoTeam opens
...
| MyBoTeam LLM Gateway         | `MYBOTEAM_GATEWAY_URL` (build-env)                      | **Free build only:** proxies LLM calls so users spend MyBoTeam credits       | `opencode serve` via proxy env injected by the private runtime |
