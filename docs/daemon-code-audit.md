...
- PID lock with stale detection (`~/.myboteam/daemon.pid`)
...
**Nobody starts it.** The Electron app does not depend on `@myboteam/daemon`, does not import from it, and does not spawn it. It exists as an independent workspace package but has no integration point.
