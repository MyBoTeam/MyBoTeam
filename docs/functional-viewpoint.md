# Functional Viewpoint — MyBoTeam Architecture
...
graph TB
  ...
  subgraph ELECTRON["MyBoTeam Desktop App (apps/desktop)"]
    direction TB
...
3. **MyBoTeam never calls LLMs directly.** Each per-task `opencode serve` orchestrates the LLM conversation and tool execution. MyBoTeam's role is configuration, gating, completion enforcement, persistence, and UI.
...
graph TB
  ...
  subgraph BRIDGE["Preload Bridge"]
    CB["contextBridge<br/><i>window.myboteam</i>"]
  end
...
**It is OpenCode's own HTTP server, not one MyBoTeam wrote.**

`opencode` (the [opencode-ai npm package](https://www.npmjs.com/package/opencode-ai)) ships a `serve` subcommand that boots a local HTTP + Server-Sent-Events server inside the opencode runtime. That server exposes the v2 API — sessions, prompts, events, permissions, questions, tool-part streams — which is exactly what [`@opencode-ai/sdk/v2`](https://www.npmjs.com/package/@opencode-ai/sdk) is built to talk to. MyBoTeam's daemon does not implement any of this protocol; it only:
...
| **Port hygiene and multi-profile** | Static port collides with co-running MyBoTeam profiles, other apps, or a crashed daemon.                                                                                                                                | `--port=0` gets a fresh ephemeral port per child.                                         |
...
graph TB
  subgraph SOURCES["Skill Sources"]
    ...
    USER_DIR["User Skills<br/><i>~/.../MyBoTeam/skills/</i>"]
...
## 9. Free-tier Gateway Integration (`@myboteam/llm-gateway-client`)

MyBoTeam ships in two flavours: the **OSS build** (open-source, bring-your-own provider keys) and the **Free build** (adds an MyBoTeam-operated LLM gateway with metered credits). The Free build is produced by a separate CI repo that fuses this repo with a private sibling package, `@myboteam/llm-gateway-client`. The OSS codebase — this repo — treats that private package as an **optional runtime dependency**: absent in OSS builds, present in Free builds, and wired in via a single interface with a null-object fallback.
...
graph TB
  subgraph BUILD["Build-time boundary"]
    ...
    CI_FREE["Free build (private CI)<br/><i>this repo + llm-gateway-client<br/>+ myboteam-release</i>"]
    BUILD_ENV["build.env<br/><i>MYBOTEAM_GATEWAY_URL=...</i>"]
  end

  subgraph RUNTIME["Runtime boundary (daemon process)"]
    direction TB
    BOOT["daemon/index.ts bootstrap"]
    DYN["Dynamic import<br/><i>await import('@myboteam/llm-gateway-client')</i>"]
    NOOP["noopRuntime<br/><i>fail-closed fallback<br/>isAvailable()=false</i>"]
    REAL["createRuntime()<br/><i>real MyboteamRuntime impl</i>"]
    TAG["setProxyTaskId(taskId &#124; undefined)<br/><i>hot-path callback</i>"]
  end

  subgraph USE["Consumers of MyboteamRuntime"]
    direction TB
    CG["ConfigGenerator<br/><i>buildMyboteamAiConfig(ctx)</i>"]
    RPC["Daemon RPC methods<br/><i>myboteam-ai.connect<br/>.get-usage · .disconnect<br/>.usage-update notify</i>"]
    ADAPT["OpenCodeAdapter.setProxyTaskId<br/><i>start → tag · teardown → clear</i>"]
  end

  subgraph EXT["External"]
    GW["MyBoTeam LLM Gateway<br/><i>HTTPS proxy to AI providers<br/>per-task credit accounting</i>"]
...
The private package's name appears on **only four lines** of OSS source. Everything else depends on the `MyboteamRuntime` _interface_ owned by agent-core.
...
| [apps/daemon/src/types/gateway-client.d.ts](apps/daemon/src/types/gateway-client.d.ts) | Ambient `declare module` so TypeScript can type `import('@myboteam/llm-gateway-client')` when the package is absent.                                                                                             |
...
### 9.2 The interface — `MyboteamRuntime` + `noopRuntime`

Defined in [`packages/agent-core/src/opencode/myboteam-runtime.ts`](packages/agent-core/src/opencode/myboteam-runtime.ts) and re-exported from [`agent-core`](packages/agent-core/src/index.ts):

| Method                             | When called                                                                                                             |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `connect(storageDeps)`             | `myboteam-ai.connect` RPC — user clicks "Use MyBoTeam AI" in Settings                                                   |
| `disconnect()`                     | `myboteam-ai.disconnect` RPC — user logs out                                                                            |
| `getUsage()`                       | `myboteam-ai.get-usage` RPC — Settings reads live credit balance                                                        |
| `onUsageUpdate(listener)`          | Daemon startup — subscribes to push updates from response headers                                                       |
| `buildProviderConfig(storageDeps)` | Per-task startup — `buildMyboteamAiConfig` asks for the opencode provider config to route LLM calls through the gateway |
| `isAvailable()`                    | Everywhere — null-object predicate, `false` in OSS, `true` in Free                                                      |

`noopRuntime` is a fail-closed implementation shipped with agent-core. `isAvailable()` returns `false`, the async methods throw `myboteam_runtime_unavailable`, `buildProviderConfig()` returns empty. Call sites never need `if (runtime) { ... }` branches — they rely on `isAvailable()` or let the null-object's empty return silently drop the integration.
...
build.env (Free CI) or build.env.template (local Free dev)
    ↓
getBuildConfig().myboteamGatewayUrl         [apps/desktop/.../build-config.ts:95]
    ↓
daemonEnv.MYBOTEAM_GATEWAY_URL = bc.myboteamGatewayUrl
    ↓ (spawned daemon inherits env)          [apps/desktop/.../daemon-connector.ts:201]
process.env.MYBOTEAM_GATEWAY_URL            (read by llm-gateway-client at createRuntime())
```
...
1. Clone `llm-gateway-client` as a sibling folder to `myboteam/`.
2. `pnpm -F @myboteam/daemon add @myboteam/llm-gateway-client@file:/Users/…/dev/myboteam/llm-gateway-client`
3. Set `MYBOTEAM_GATEWAY_URL=<dev-gateway>` in `build.env` (or inline on the `pnpm dev` command).
4. `pnpm dev` — the daemon's dynamic `import()` now resolves the local package; all four consumer paths light up.

Reverting to OSS mode is `pnpm -F @myboteam/daemon remove @myboteam/llm-gateway-client` + restart. No other code changes required — that's the whole point of the null-object pattern.
...
| **MyboteamRuntime (interface)**    | agent-core   | Null-object (`noopRuntime`) in OSS; real impl loaded from private `@myboteam/llm-gateway-client` in Free (see §9) | `connect()`, `disconnect()`, `getUsage()`, `onUsageUpdate()`, `buildProviderConfig()`, `isAvailable()`            |
...
| **Database**                       | agent-core   | SQLite WAL + migrations + repositories                                                                            | `sql.js` via repositories                                                                                 |
...
| **Preload Bridge**                 | apps/desktop | `contextBridge.exposeInMainWorld('myboteam', ...)`                                                                | ~70 API methods exposed to renderer                                                                               |
