...
This document captures all stateful data, information models, state machines, and data flows within the MyBoTeam system. It follows the Rozanski & Woods Information Viewpoint to document data ownership, lifecycle, and structure.
...
- SQLite DB: `~/Library/Application Support/MyBoTeam/myboteam.db` (prod) / `myboteam-dev.db` (dev)
- Secure Storage: `~/Library/Application Support/MyBoTeam/secure-storage.json`
- Logs: `~/Library/Application Support/MyBoTeam/logs/`
- Skills: `~/Library/Application Support/MyBoTeam/skills/`
- OpenCode configs: `~/Library/Application Support/MyBoTeam/opencode/`
...
Location: `~/Library/Application Support/MyBoTeam/` (macOS)

All files and folders found in this directory at runtime:

### MyBoTeam Application Data

| Path                  | Owner               | Description                                                                                                                                                                         |
| --------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `myboteam.db`         | agent-core (SQLite) | Main database — tasks, messages, todos, providers, settings, skills, connectors. All application state.                                                                             |
| `myboteam.db-wal`     | SQLite engine       | Write-Ahead Log — buffered writes not yet checkpointed to main DB. Auto-managed.                                                                                                    |
| `myboteam.db-shm`     | SQLite engine       | Shared memory index for WAL. Enables concurrent readers. Auto-managed.                                                                                                              |
| `myboteam-dev.db`     | agent-core (SQLite) | Development-mode database (used when running `pnpm dev`). Same schema as prod.                                                                                                      |
| `myboteam-dev.db-wal` | SQLite engine       | WAL for dev database.                                                                                                                                                               |
| `myboteam-dev.db-shm` | SQLite engine       | Shared memory for dev database.                                                                                                                                                     |
...
These are standard Chromium storage directories created automatically by Electron. Not managed by MyBoTeam code.

| Path                                                  | Owner           | Description                                                                                      |
| ----------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------ |
| `Cache/`                                              | Chromium        | HTTP cache for the renderer process (the MyBoTeam UI webview).                                   |
| `Code Cache/`                                         | Chromium        | Compiled/cached JavaScript bytecode for faster UI startup.                                       |
| `Cookies`                                             | Chromium        | Cookie database for the Electron renderer (MyBoTeam UI, not the automation browser).             |
...
| `IndexedDB/`                                          | Chromium        | Browser IndexedDB storage for the MyBoTeam UI renderer.                                          |
| `Local Storage/`                                      | Chromium        | `localStorage` for the MyBoTeam React app (theme preference synced here).                        |
| `Session Storage/`                                    | Chromium        | `sessionStorage` for the MyBoTeam React app.                                                     |
...
| Scenario                           | Look at                                                                           |
| ---------------------------------- | --------------------------------------------------------------------------------- |
| Task not completing / wrong status | `myboteam.db` → `tasks` table (status, session_id)                                |
| Messages missing or corrupt        | `myboteam.db` → `task_messages` table (sort_order, content)                       |
| Provider connection issues         | `myboteam.db` → `providers` table (connection_status, credentials_type)           |
