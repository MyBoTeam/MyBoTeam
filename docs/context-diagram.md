# MyBoTeam — System Context Diagram
...
graph TB
    ...
    MyBoTeam["<b>MyBoTeam</b><br/><i>[Software System]</i><br/><br/>AI desktop agent that automates<br/>tasks via natural language"]:::system
    ...
    User -->|"natural language tasks<br/>+ approvals"| MyBoTeam
    MyBoTeam -->|"progress · results<br/>· permission requests"| User
    MyBoTeam <-->|"LLM prompts / responses"| AI
    MyBoTeam <-->|"read · write · organize"| FS
    MyBoTeam -->|"browse · click · extract"| BROWSER
    MyBoTeam <-->|"remote tool calls"| MCP
    MyBoTeam -->|"voice input"| VOICE
    MyBoTeam -->|"download skills"| SKILLS
```
...
graph TB
    ...
    MyBoTeam["<b>MyBoTeam</b><br/><i>[Software System]</i><br/><br/>AI desktop agent that automates<br/>file management, document creation,<br/>and browser tasks locally"]:::system
    ...
    User -- "Describes tasks,<br/>approves file operations,<br/>answers questions" --> MyBoTeam
    MyBoTeam -- "Shows progress,<br/>asks permissions,<br/>presents results" --> User

    MyBoTeam -- "Sends prompts,<br/>receives AI responses<br/>(via OpenCode CLI)" --> CloudAI
    MyBoTeam -- "Sends prompts,<br/>receives AI responses<br/>(via OpenCode CLI)" --> LocalAI

    MyBoTeam -- "Reads, creates, moves,<br/>renames, deletes files<br/>(with user permission)" --> FileSystem

    MyBoTeam -- "Launches, navigates,<br/>clicks, fills forms,<br/>extracts data" --> ChromeBrowser
    ChromeBrowser -- "Accesses" --> SaaS

    MyBoTeam -- "Connects via OAuth 2.0,<br/>calls remote tools" --> MCPRemote

    MyBoTeam -- "Transcribes<br/>voice input" --> ElevenLabs
    MyBoTeam -- "Downloads<br/>skill definitions" --> GitHub
```
...
1. **Local-first**: MyBoTeam runs entirely on the user's machine. No MyBoTeam backend exists. Data stays local.

2. **Bring-your-own-AI**: Users provide their own API keys or run local models. MyBoTeam is a tool, not a service.
