# Development Viewpoint — MyBoTeam Architecture
...
```mermaid
graph TD
  subgraph MONOREPO["MyBoTeam Monorepo (pnpm workspaces)"]
    direction TB

    DESKTOP["@myboteam/desktop<br/><i>Electron app · v0.3.8</i><br/>apps/desktop/"]
    WEB["@myboteam/web<br/><i>React UI · v0.0.1</i><br/>apps/web/"]
    CORE["@myboteam/agent-core<br/><i>Shared library · v0.4.0</i><br/>packages/agent-core/"]
...
graph TB
  ...
  subgraph MAIN_LAYER["Application Layer — Electron Main Process"]
    direction LR
    ELECTRON["Electron 35"] ~~~ SQL_JS["sql.js (WASM)"] ~~~ NODE_PTY["node-pty"] ~~~ ZOD["Zod 3"] ~~~ CRYPTO["AES-256-GCM<br/>(secure storage)"]
  end
  ...
  class ELECTRON,SQL_JS,NODE_PTY,ZOD,CRYPTO main
...
| **Apps**             | `@myboteam/desktop` (Electron 35), `@myboteam/web` (React 19)       |
| **Shared library**   | `@myboteam/agent-core` (TypeScript ESM)                             |
...
| **Database**         | SQLite (sql.js WASM) + AES-256-GCM encrypted secure storage      |
