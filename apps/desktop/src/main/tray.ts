...
import { disableAutoStart, enableAutoStart, isAutoStartEnabled } from './daemon/service-manager';
...
export function updateTaskCount(count: number, mainWindow: BrowserWindow | null): void {
  ...
  if (tray && !tray.isDestroyed()) {
    tray.setContextMenu(buildContextMenu(mainWindow));
    const tooltip = count > 0 ? `MyBoTeam — ${count} task(s) running` : 'MyBoTeam';
