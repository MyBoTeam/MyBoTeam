...
import type { ProviderId } from '@myboteam/agent-core/desktop-main';
...
export async function startApp(
  ...
  // HuggingFace auto-start + myboteam-ai cleanup used to run here in the
  ...
  // `myboteam-ai` under the free tier, the old pre-import cleanup would
  ...
  // `bootstrapDaemon()` (the RPCs need a live client).
  ...
  try {
    ...
    // Clean up stale myboteam-ai provider if free mode is no longer
