 ...
 *
 * URL comes from build-config (`MYBOTEAM_UPDATER_URL`, CI-injected in Free builds
 * or opt-in via .env in dev). Manifest names are hardcoded per the myboteam-release
 * upload contract — do not parameterize.
 *
 * See: myboteam-release/scripts/upload-r2-{windows,macos,linux}.sh
 */
...
/**
 * Manifest filenames as published by myboteam-release. DO NOT parameterize —
