 ...
 *
 * Manifests may carry absolute download URLs (e.g. `https://downloads.myboteam.app/...`)
 * on a different subdomain than the manifest host (`https://d.myboteam.app/...`), so a
 * strict same-origin check is too tight. Instead we accept any URL whose hostname shares
 * the feed URL's apex (last two labels) — covers `downloads.myboteam.app` vs
 * `d.myboteam.app` without accepting `evil.example.com`.
 *
 ...
 * "same apex" and accept `bar.co.uk` (unrelated customer). Fine for `myboteam.app`, but
...
export function isSameApex(candidate: string, reference: string): boolean {
  ...
  // `http://downloads.myboteam.app/...` would otherwise hand the user a
...
function getApex(hostname: string): string {
  ...
  if (labels.length <= 2) {
    // 'localhost' or a bare apex like 'myboteam.app' — use as-is.
    return hostname;
  }
