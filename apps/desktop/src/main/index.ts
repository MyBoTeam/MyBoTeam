 ...
 *                       rmSync.
 *   - `exited`        — we connected to an MyBoTeam daemon, sent it
 *                       `daemon.shutdown`, and observed the socket
 *                       close within the drain window. Safe to rmSync.
 *   - `still-alive`   — we connected to a live MyBoTeam daemon but
...
async function stopDetachedDaemonForCleanStart(
  ...
  // connect proves the peer is an MyBoTeam daemon for THIS userData.
...
if (process.env.CLEAN_START === '1') {
  ...
  if (shutdownState === 'still-alive') {
    ...
    // that an MyBoTeam daemon owns this profile and it didn't exit
