...
function registerNotificationHandlers(
  ...
  // subscribes via `myboteam.onAuthError` in preload.
  ...
  // subscribes via `myboteam.onBrowserFrame` in preload.
  client.onNotification('browser.frame', (data) => {
    forward('browser:frame', data);
  });

  // MyBoTeam AI credit usage updates (proxy → daemon → Electron → renderer)
