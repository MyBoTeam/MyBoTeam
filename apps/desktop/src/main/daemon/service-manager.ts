...
function getDaemonNodePath(): string {
  if (app.isPackaged) {
    ...
    // Search versioned subdirectory (e.g. node-v24.15.0-win-x64)
    try {
...
const LAUNCH_AGENT_LABEL = 'ai.myboteam.daemon';
...
function getLaunchAgentContent(): string {
  ...
  // Electron's environment, so MYBOTEAM_APP_PATH would otherwise be missing.
...
const SYSTEMD_SERVICE_NAME = 'myboteam-daemon.service';
