 ...
 * worked because native addons were ABI-compatible
 * to match Electron's ABI via `@electron/rebuild` postinstall.
 *
 * Post-M6 (daemon-only-SQLite migration): the daemon uses `sql.js` (WASM)
 * sql.js uses WASM, so native ABI matching is no longer needed.
 * Desktop no longer runs `@electron/rebuild`. However, spawning Electron-as-Node
 * still fails for other native modules in the daemon's dependency tree.
 *
 * Preference order:
 *   1. Bundled Node under `resources/nodejs/<platform>-<arch>/…` —
 *      same binary the packaged app uses. Requires
 *      `pnpm -F @myboteam/desktop download:nodejs` to have been run;
 ...
 *      `apps/daemon/`, so its ABI matches whatever native addons
 *      in `apps/daemon/node_modules/` were built against. Reliable
 *      when pnpm is on the shell's PATH.
...
function getDevNodePath(): string {
  try {
    ...
    // Bundled Node not downloaded yet — fall through.
  }
...
export function spawnDaemon(dataDir: string): void {
  // M6 invariant: spawn under real Node (bundled in packaged mode, same
  // or system Node in dev) — never Electron-as-Node. The daemon uses
  // `sql.js` (WASM, no ABI concerns), but other native deps may require
  // real Node's ABI rather than Electron's.
  ...
  // this for dev mode; post-M6 we must ensure it's NOT set even if
  // the parent Electron process inherited it from its own shell.
  delete daemonEnv.ELECTRON_RUN_AS_NODE;

  // Inject gateway URL so the daemon can start the MyBoTeam AI proxy.
