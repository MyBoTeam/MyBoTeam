...
## Saving Skills in MyBoTeam

**IMPORTANT:** When creating skills in MyBoTeam, you can ONLY create "custom" skills. You CANNOT create "official" skills - those are bundled with the app and managed by the MyBoTeam team.
...
Skills must be saved to the MyBoTeam user data directory under a `skills` folder:

**macOS:** `~/Library/Application Support/MyBoTeam/skills/<skill-name>/SKILL.md`
**Windows:** `%APPDATA%\MyBoTeam\skills\<skill-name>\SKILL.md`
**Linux:** `~/.config/MyBoTeam/skills/<skill-name>/SKILL.md`
...
1. **Use the base path from the task prompt** when present.
   - If no path is provided, detect the OS and use:
     - macOS: `~/Library/Application Support/MyBoTeam/skills/`
     - Windows: `%APPDATA%\MyBoTeam\skills\`
     - Linux: `~/.config/MyBoTeam/skills/`
...
5. **The skill is automatically detected** - MyBoTeam scans this directory on startup and syncs new skills to its database. The skill will appear in Settings > Skills as a "Custom" skill.

### Skill Frontmatter Rules

For custom skills in MyBoTeam:
