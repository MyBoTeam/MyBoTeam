 ...
 * here. The daemon uses `sql.js` (WASM) via `scripts/stage-daemon-deps.cjs`.
 */
...
try {
  ...
  // Remove empty @myboteam directory if it exists
  ...
  // longer ships native modules. `build.npmRebuild: false` in
  ...
  if (packagesToRestore.length > 0) {
    ...
    // Recreate @myboteam directory if needed
