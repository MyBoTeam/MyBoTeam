...
vi.mock('electron', () => ({
  app: { getVersion: vi.fn(() => '0.3.8'), isPackaged: true, name: 'MyBoTeam' },
  dialog: {
    showMessageBox: vi.fn(() => Promise.resolve({ response: 2 })),
    showErrorBox: vi.fn(),
  },
...
describe('manual-manifest', () => {
  beforeEach(async () => {
    ...
    mockHttpsGet.mockImplementation((_url: string, cb: (res: unknown) => void) => {
      cb(
        mockHttpResponse(
          200,
          `version: 1.0.0\npath: https://downloads.myboteam.app/downloads/1.0.0/windows/MyBoTeam-1.0.0.exe\nsha512: abc\nreleaseDate: '2026-01-01'\n`,
        ),
      );
      return { on: vi.fn() };
    });
  ...
  describe('release-contract URL (latest-<platform>.yml, no channel suffix)', () => {
    ...
    it('Linux x64 fetches latest-linux.yml', async () => {
      ...
      mockHttpsGet.mockImplementation((url: string, cb: (res: unknown) => void) => {
        fetched = url;
        cb(mockHttpResponse(200, `version: 0.3.8\npath: https://downloads.myboteam.app/a.deb\n`));
        return { on: vi.fn() };
      });
      const { checkForUpdatesManual } = await import('../../../src/main/updater/manual-manifest');
      await checkForUpdatesManual(true, 'linux', 'x64');
      expect(fetched).toBe('https://d.myboteam.app/latest-linux.yml');
    });
    ...
    it('Linux arm64 fetches latest-linux-arm64.yml', async () => {
      ...
      expect(fetched).toBe('https://d.myboteam.app/latest-linux-arm64.yml');
    });
    ...
    it('trailing slash in URL does not produce a double-slash', async () => {
      ...
      vi.mocked(getBuildConfig).mockReturnValue({
        ...emptyConfig,
        myboteamUpdaterUrl: 'https://d.myboteam.app/test/',
      });
      let fetched = '';
      mockHttpsGet.mockImplementation((url: string, cb: (res: unknown) => void) => {
        fetched = url;
      ...
      const { checkForUpdatesManual } = await import('../../../src/main/updater/manual-manifest');
      await checkForUpdatesManual(true, 'win');
      expect(fetched).toBe('https://d.myboteam.app/test/latest-win.yml');
    });
  });
  ...
  describe('manifest parsing (js-yaml)', () => {
    ...
    it('YAML missing version: tracked as invalid_manifest', async () => {
      mockHttpsGet.mockImplementation((_url: string, cb: (res: unknown) => void) => {
        cb(mockHttpResponse(200, `path: https://downloads.myboteam.app/a.exe\nsha512: abc\n`));
        return { on: vi.fn() };
      });
      const analytics = await import('../../../src/main/analytics/events');
      const { checkForUpdatesManual } = await import('../../../src/main/updater/manual-manifest');
    ...
    it('unparseable remote version silent: still tracked, no dialog', async () => {
      mockHttpsGet.mockImplementation((_url: string, cb: (res: unknown) => void) => {
        cb(
          mockHttpResponse(200, `version: "garbage"\npath: https://downloads.myboteam.app/a.exe\n`),
        );
        return { on: vi.fn() };
      });
      const analytics = await import('../../../src/main/analytics/events');
  ...
  describe('manifest path origin validation', () => {
    ...
    it('accepts same-apex subdomain (downloads.myboteam.app vs feed d.myboteam.app)', async () => {
      mockHttpsGet.mockImplementation((_url: string, cb: (res: unknown) => void) => {
        cb(
          mockHttpResponse(
            200,
            `version: 9.9.9\npath: https://downloads.myboteam.app/downloads/9.9.9/windows/a.exe\n`,
          ),
        );
        return { on: vi.fn() };
      });
  ...
  describe('dialog routing', () => {
    ...
    it('Download button opens external URL from manifest', async () => {
      ...
      expect(shell.openExternal).toHaveBeenCalledWith(
        'https://downloads.myboteam.app/downloads/1.0.0/windows/MyBoTeam-1.0.0.exe',
      );
    });
