...
describe('updater', () => {
  ...
  describe('feed-config', () => {
    it('getFeedUrl trims a single trailing slash', async () => {
      ...
      expect(getFeedUrl()).toBe('https://d.myboteam.app/test');
    });
    ...
    it('getManifestName pins the myboteam-release contract', async () => {
      ...
      try {
        mockHttpsGet.mockImplementation((_url: string, cb: (res: unknown) => void) => {
          ...
          setTimeout(() => {
            res.emit(
              'data',
              `version: 0.3.8\npath: https://downloads.myboteam.app/downloads/0.3.8/windows/a.exe\n`,
            );
            res.emit('end');
          }, 0);
          cb(res);
  ...
  describe('event handlers (darwin)', () => {
    ...
    it('update-available handler rejects untrusted native download URLs before download', async () => {
      ...
      const { dialog } = await import('electron');
      try {
        const { initUpdater, checkForUpdates, getUpdateState } = await import(
          '../../../src/main/updater'
        );
        await initUpdater(mockWindow);
        await checkForUpdates(false);
    ...
    it('update-downloaded handler records downloaded version and invokes callback', async () => {
      const restore = setPlatform('darwin');
      try {
        const { initUpdater, getUpdateState, setOnUpdateDownloaded } = await import(
          '../../../src/main/updater'
        );
