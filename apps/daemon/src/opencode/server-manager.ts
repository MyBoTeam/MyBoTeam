...
export interface ServerManagerDeps extends TaskConfigBuilderOptions {
  storage: StorageAPI;
  /** Optional MyBoTeam runtime context (proxy, credits, etc.); daemon wires it. */
...
function spawnOpenCodeServer(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onExit = (code: number | null): void => {
      close();
      notifyClosed();
      // Log at info level when the server already settled (expected cleanup
      // after task complete), warn level for unexpected early exits.
