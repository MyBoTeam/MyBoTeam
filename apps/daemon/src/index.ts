...
async function main(): Promise<void> {
  ...
  // ── Load MyBoTeam AI runtime (noop in OSS, real impl in commercial) ───
  ...
  const storage = await storageService.initialize(dataDir);
  ...
  const userDataPath = dataDir || path.join(homedir(), '.myboteam');
  const isPackaged = args.isPackaged || process.env.MYBOTEAM_IS_PACKAGED === '1';
  const resourcesPath = args.resourcesPath || process.env.MYBOTEAM_RESOURCES_PATH || '';
  const appPath = args.appPath || process.env.MYBOTEAM_APP_PATH || '';
  ...
  // not installed. Mirrors the `myboteamRuntime` bootstrap pattern at
