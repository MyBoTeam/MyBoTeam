...
import { rowFromResult, rowsFromResult } from '@myboteam/agent-core/storage/query-helpers';
...
export class GoogleAccountService extends EventEmitter {
  ...
  private async refreshToken(accountId: string): Promise<void> {
    ...
    try {
      ...
      this.db.run('UPDATE google_accounts SET last_refreshed_at = ? WHERE google_account_id = ?', [
  ...
  private handlePermanentFailure(accountId: string): void {
    this.cancelRefresh(accountId);
    this.db.run("UPDATE google_accounts SET status = 'expired' WHERE google_account_id = ?", [
      accountId,
    ]);
    flushDatabase();
  ...
  private isDuplicate(googleAccountId: string): boolean {
    const row = rowFromResult<{ '1': number }>(
      this.db.exec('SELECT 1 FROM google_accounts WHERE google_account_id = ?', [googleAccountId]),
    );
  ...
  private isDuplicateLabel(label: string, excludeGoogleAccountId?: string): boolean {
    ...
    const row = rowFromResult<{ '1': number }>(
      this.db.exec('SELECT 1 FROM google_accounts WHERE LOWER(label) = LOWER(?)', [label]),
    );
    return row !== undefined;
  }
}
