...
describe('OpenAiOauthManager', () => {
  ...
  it('startLogin falls back to any OAuth method when preferred label is absent', async () => {
    // Return methods without "ChatGPT Pro/Plus" — should pick the
    // remaining oauth-type entry at index 1 instead of throwing.
    ...
    expect(authorizeUrl).toBe('https://auth.openai.com/login?client_id=fake');
    // Should have picked the oauth method at index 1 (not 0, since
    // index 0 is 'api').
    expect(oauthAuthorizeMock).toHaveBeenCalledWith(
  ...
  it('abortActive is safe when no session is active', () => {
    // Private, but tested via the public dispose path which calls
    // abortActive internally. dispose with no active session should
    // be a no-op.
    const manager = new OpenAiOauthManager({} as never);
    // First dispose clears any active session; second dispose should
    // hit the early return.
    manager.dispose();
    manager.dispose();
    // If no throw we're good.
  });
});
