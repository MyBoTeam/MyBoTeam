...
describe('SettingsService', () => {
  ...
  it('setSandboxConfig, setCloudBrowserConfig, setMessagingConfig each emit', () => {
    const changes = captureChanges(service);

    service.setSandboxConfig({ enabled: true } as never);
    expect(changes).toEqual([{ key: 'sandboxConfig', value: { enabled: true } }]);

    service.setCloudBrowserConfig({ enabled: true } as never);
    expect(changes).toEqual([
      { key: 'sandboxConfig', value: { enabled: true } },
      { key: 'cloudBrowserConfig', value: { enabled: true } },
    ]);

    service.setMessagingConfig({ integrations: {} } as never);
    expect(changes).toEqual([
      { key: 'sandboxConfig', value: { enabled: true } },
      { key: 'cloudBrowserConfig', value: { enabled: true } },
      { key: 'messagingConfig', value: { integrations: {} } },
    ]);
  });
