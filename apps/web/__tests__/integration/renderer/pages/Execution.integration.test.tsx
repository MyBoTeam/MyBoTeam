...
describe('Execution Page Integration', () => {
  beforeEach(() => {
    ...
    (window as Window & { myboteam: typeof mockMyBoTeam }).myboteam = mockMyBoTeam;
    // Reset store state
    mockStoreState = {
      currentTask: null,
      loadTaskById: mockLoadTaskById,
  ...
  describe('message display', () => {
    ...
    it('should show thinking indicator when running without tool', () => {
      ...
      expect(
        screen.getByText(/^(Doing|Executing|Running|Handling it|MyBoTeaming)\.\.\.$/),
      ).toBeInTheDocument();
    });
  ...
  describe('permission dialog', () => {
    ...
    it('should call respondToPermission with custom text for question permission', async () => {
      ...
      const textarea = screen.getByPlaceholderText(/enter a different option/i);
      fireEvent.change(textarea, { target: { value: 'continue' } });

      const submitButton = screen.getByTestId('permission-allow-button');
      fireEvent.click(submitButton);

      await waitFor(() => {
        expect(mockRespondToPermission).toHaveBeenCalled();
      });
    });
  ...
  describe('task complete states', () => {
    ...
    it('should show task cancelled message for cancelled task', () => {
      ...
      // Assert - cancelled tasks render a badge span and heading
      const cancelledElements = screen.getAllByText(/cancelled/i);
    ...
    it('should authenticate Slack and resume the task from the assistant action button', async () => {
      ...
      mockStoreState.currentTask = task;
      // First call consumed by useExecutionEffects effect on mount
