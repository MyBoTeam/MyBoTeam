...
describe('TaskInputBar Integration', () => {
  ...
  describe('slash command integration', () => {
    ...
    beforeEach(() => {
      vi.mocked(mockMyBoTeam.getEnabledSkills).mockResolvedValue(mockSkills);
    });

    afterEach(() => {
      vi.mocked(mockMyBoTeam.getEnabledSkills).mockResolvedValue([]);
    });
  ...
  describe('plus menu integration', () => {
    it('should keep skills search open when clicking search input in plus menu', async () => {
      ...
      (window as Window & { myboteam: Record<string, unknown> }).myboteam = {
        getEnabledSkills: vi.fn().mockResolvedValue([
          {
            id: 'skill-git-helper',
            name: 'Git Helper',
