...
export function registerTaskSubscriptions(getStore: () => import('./taskStore').TaskState) {
  ...
  window.myboteam.onTaskProgress((progress: unknown) => {
  ...
  window.myboteam.onTaskUpdate((event: unknown) => {
  ...
  window.myboteam.onTodoUpdate?.((data: { taskId: string; todos: TodoItem[] }) => {
    const state = getStore();
  ...
  window.myboteam.onAuthError?.((data: { providerId: string; message: string }) => {
    getStore().setAuthError(data);
  });

  window.myboteam.onDaemonReconnected(() => {
    const state = getStore();
    void state.loadTasks();
  ...
  window.myboteam.onWorkspaceChanged?.(async () => {
    const state = getStore();
    state.reset();
    try {
