...
export function useBedrockProviderConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await myboteam.validateBedrockCredentials(credentials);
      ...
      await myboteam.saveBedrockCredentials(credentials);

      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await myboteam.fetchBedrockModels(credentialsJson);
