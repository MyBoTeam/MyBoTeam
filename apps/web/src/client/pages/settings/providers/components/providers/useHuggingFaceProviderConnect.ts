...
export function useHuggingFaceProviderConnect({
  ...
  useEffect(() => {
    const myboteam = getMyBoTeam();
    const unsub = myboteam.onHuggingFaceDownloadProgress((progress) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const downloadResult = await myboteam.downloadHuggingFaceModel(selectedModelId);
      ...
      const serverResult = await myboteam.startHuggingFaceServer(selectedModelId);
