...
export function useAddSkill(): UseAddSkillResult {
  ...
  const handleUploadSkill = async (onSkillAdded?: () => void) => {
    if (isLoadingRef.current || !window.myboteam) {
      return;
    }
    try {
      ...
      setIsLoading(true);
      setUploadError(null);
      const folderPath = await window.myboteam.pickSkillFolder();
      if (!folderPath) {
        setIsLoading(false);
        return;
      }
      await window.myboteam.addSkillFromFolder(folderPath);
      onSkillAdded?.();
    } catch (err) {
  ...
  const handleImportFromGitHub = async (onSkillAdded?: () => void) => {
    ...
    if (isLoadingRef.current || !normalizedGitHubUrl || !window.myboteam) {
      return;
    }
    try {
      isLoadingRef.current = true;
      setIsLoading(true);
      setError(null);
      await window.myboteam.addSkillFromGitHub(normalizedGitHubUrl);
      setGitHubUrl('');
