...
export function SandboxSection({ visible }: SandboxSectionProps) {
  ...
  const handleToggle = useCallback(async () => {
    ...
    await myboteam.setSandboxConfig(updated);
    setSandboxConfig(updated);
    setSandboxEnabled(newEnabled);
  }, [sandboxEnabled, sandboxConfig, myboteam]);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="ml-4">
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ease-myboteam ${
              sandboxEnabled ? 'bg-primary' : 'bg-muted'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ease-myboteam ${
