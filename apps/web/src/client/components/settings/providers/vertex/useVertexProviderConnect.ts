...
export function useVertexProviderConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const validation = await myboteam.validateVertexCredentials(credentials);
      ...
      await myboteam.saveVertexCredentials(credentials);
      const credentialsJson = JSON.stringify(credentials);
      const modelsResult = await myboteam.fetchVertexModels(credentialsJson);
