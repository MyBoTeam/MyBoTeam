...
export function useLMStudioProviderConnect({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const result = await myboteam.testLMStudioConnection(serverUrl);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const result = await myboteam.testLMStudioConnection(currentUrl);
      ...
      if (latestProvider?.connectionStatus !== 'connected') {
        return;
      }
