...
if (hostNodeTarget) {
  if (!isHostNodeRuntimeAvailable(hostNodeTarget)) {
    ...
    runPnpm(
      ['-F', '@myboteam/desktop', 'download:nodejs', `--platform=${hostNodeTarget}`],
...
if (needsAgentCoreBuild) {
  console.log('Missing @myboteam/agent-core build outputs:');
  ...
  runPnpm(['-F', '@myboteam/agent-core', 'build'], 'Building @myboteam/agent-core...');

  missingOutputs = [...outputPaths].filter(
    (relativeOutputPath) => !fs.existsSync(path.join(agentCoreDir, relativeOutputPath)),
  );
  if (missingOutputs.length > 0) {
    console.error('Failed to produce required @myboteam/agent-core outputs:');
    for (const missingOutput of missingOutputs) {
...
if (missingMcpDistOutputs.length > 0) {
  ...
  runPnpm(
    ['-F', '@myboteam/desktop', 'build:mcp-tools:dev'],
...
console.log('✓ @myboteam/agent-core and MCP dist outputs found');
process.exit(0);
