...
if (!url) {
  ...
  console.error('Example: pnpm dev:remote https://myboteam-app-preview-42.myboteam.workers.dev');
  process.exit(1);
}

const env = {
  ...process.env,
  MYBOTEAM_ROUTER_URL: url,
};

console.log('[dev:remote] Launching Electron → ' + url);

const electronArgs = ['-F', '@myboteam/desktop', 'dev:remote'];
if (isCheck) {
