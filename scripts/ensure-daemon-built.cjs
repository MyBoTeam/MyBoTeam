...
if (needsBuild()) {
  console.log('Building @myboteam/daemon for dev...');
  try {
    runPnpmSync(['-F', '@myboteam/daemon', 'build'], {
      cwd: rootDir,
      env: process.env,
      stdio: 'inherit',
    });
    ...
    console.error(`Failed to build @myboteam/daemon: ${message}`);
    process.exit(1);
  }
...
console.log('✓ @myboteam/daemon build output found');
process.exit(0);
